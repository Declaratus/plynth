...
- **`msdyn_PssCreateV1` / `PssCreateV2`** — create scheduling entities inside an OperationSet. V2 supports multiple entities per call. GA.
...
- **Dataverse service protection limits** (separate, per-user, 5-minute sliding window, per `learn.microsoft.com/en-us/power-apps/developer/data-platform/api-limits`): ~6,000 requests, ~20 min combined execution time, ~52 concurrent; 429 + Retry-After on breach. A `$batch` can hold up to 1,000 operations but each still counts as a request.
